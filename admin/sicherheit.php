...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'save_notfound') {
    ...
    access_code_delete(trim($_POST['code'] ?? ''));
...
<div class="admin-section">
  <div class="admin-head-row" style="margin-bottom:1rem">
    <h2 style="margin:0">Freigabecodes</h2>
  ...
  <p style="font-size:.82rem;color:#8a8a95;margin:0 0 1rem">
    Ein Code schaltet ein Konto frei und ist <strong>einmal verwendbar</strong>. Ohne Code kann trotzdem ein Konto erstellt werden, es bleibt dann zunächst eingeschränkt.
  </p>
  <?php if (empty($codes)): ?>
  ...
  <div class="table-card"><table class="data-table">
    ...
    <tbody>
      ...
      <tr<?= $used ? ' style="opacity:.7"' : '' ?>>
        ...
        <td>
          <?php if ($owner > 0): ?>
            <span class="tag tag-ok">Kunde</span> <?= h($cd['account_name'] ?: '') ?> <span class="muted"><?= h($cd['account_email'] ?: ('#' . $owner)) ?></span>
          <?php else: ?>
